...
public class SolutionDay07_L0502 {
    ...
    public List<Interval> employeeFreeTime(List<List<Interval>> avails) {
    	...
    	for (int i = 0; i < n; ++i) {
    		...
    		int maxLeft = now.end;
    		int j = i + 1;
    		while (j < n && unSorted.get(j).start <= maxLeft) {
    			maxLeft = Math.max(maxLeft, unSorted.get(j).end);
    			j ++;
    		}
    		if (j < n) ans.add(new Interval(maxLeft, unSorted.get(j).start));
    		i = j - 1;
    	}
    	return ans;
    }
	...
	public static void main(String[] args) {
		SolutionDay07_L0502 day = new SolutionDay07_L0502();
